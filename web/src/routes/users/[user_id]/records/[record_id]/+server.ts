...
export const PUT = async ({ request, params }) => {
    try {
        ...
        await pb.collection('records').update(record_id, { "time": parseInt(body["time-spent"]), "rating": parseInt(body["rating"]), "description": body["description"], "user": user_id, "date": get_date_from_ddmmyyyy(body["date"]).toISOString(), "language": body["programming-language"] });
